...
%                   it will be 1.
%   NumberOfFrames   the total number of frames in the movie regardless of
...
%                   1/framerate.
%   NumberOfFrames   Audio comes in packets or frames when captured, the
...
if useFFGRAB
	...
	if iscell(filename)
        ...
        video = struct('Width',{},'Height',{},'NumberOfFrames',{},'frames',{});
        audio = struct('nrChannels',{},'rate',{},'bits',{},'NumberOfFrames',{},'data',{},'frames',{});
        ...
        if strcmp(matlabCommand,'')
            ...
            for i=1:nrVideoStreams
                [Width, Height, rate, nrFramesCaptured, NumberOfFrames, totalDuration] = FFGrab('getVideoInfo',i-1);
                video(i).Width = Width;
                video(i).Height = Height;
                video(i).rate = rate;
                video(i).NumberOfFrames = NumberOfFrames;
                ...
                if (NumberOfFrames > 0 && any(frames > NumberOfFrames))
                    warning('mmread:general',['Frame(s) ' num2str(frames(frames>NumberOfFrames)) ' exceed the number of frames in the movie.']);
                end
        ...
        for i=1:nrAudioStreams
            [nrChannels, rate, bits, nrFramesCaptured, NumberOfFrames, subtype, totalDuration] = FFGrab('getAudioInfo',i-1);
            audio(i).nrChannels = nrChannels;
            audio(i).rate = rate;
            audio(i).bits = bits;
            audio(i).NumberOfFrames = NumberOfFrames;
...
%                        end
    ...
    try
        ...
        if strcmp(matlabCommand,'')
            % loop through getting all of the video data from each stream
            for i=1:nrVideoStreams
                [Width, Height, rate, nrFramesCaptured, NumberOfFrames, totalDuration] = mexDDGrab('getVideoInfo',i-1);
                video(i).Width = Width;
                video(i).Height = Height;
                video(i).rate = rate;
                video(i).NumberOfFrames = NumberOfFrames;
        ...
        % loop through getting all of the audio data from each stream
        for i=1:nrAudioStreams
            [nrChannels, rate, bits, nrFramesCaptured, NumberOfFrames, subtype, totalDuration] = mexDDGrab('getAudioInfo',i-1);
            audio(i).nrChannels = nrChannels;
            audio(i).rate = rate;
            audio(i).bits = bits;
            audio(i).NumberOfFrames = NumberOfFrames;
