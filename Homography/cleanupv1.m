%{
function name: cleanupv1
input: image matrix
output: an image that contains only the arena (masks the area outside the arena)
logic: 
%}
function corners= cleanupv1(img)
    
    if nargin==0
        img=imread('photos and videos\img-sc-1.jpg');
        size(img)
    end
    ...
    x=[stats(index_xpy_max).Centroid(1) stats(index_xmy_max).Centroid(1) stats(index_xmy_min).Centroid(1) stats(index_xpy_min).Centroid(1)];
    y=[stats(index_xpy_max).Centroid(2) stats(index_xmy_max).Centroid(2) stats(index_xmy_min).Centroid(2) stats(index_xpy_min).Centroid(2)];
    corners=[x(1) x(2) x(3) x(4);y(1) y(2) y(3) y(4)];
    hold on;
